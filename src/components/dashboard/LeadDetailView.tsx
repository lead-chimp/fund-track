...
export function LeadDetailView({ leadId }: LeadDetailViewProps) {
  ...
  const handleStatusChange = useCallback(async (newStatus: LeadStatus, reason?: string) => {
    if (!lead) return
  ...
  const handleNotesUpdate = useCallback((updatedNotes: LeadNote[], newCount: number) => {
    setLead(prev => prev ? {
      ...prev,
      notes: updatedNotes,
  ...
  const downloadDocument = async (doc: Document) => {
    try {
      const response = await fetch(`/api/leads/${leadId}/documents/${doc.id}/download`)
  ...
  const deleteDocument = async (doc: Document) => {
    ...
    try {
      const response = await fetch(`/api/leads/${leadId}/files?documentId=${doc.id}`, {
        method: 'DELETE',
      })
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500">Lead ID: {lead.id} • Legacy ID: {lead.legacyLeadId}</p>
          </div>
          <div className="flex items-center space-x-3">
            <span className={`inline-flex px-3 py-1 text-sm font-semibold rounded-full ${STATUS_COLORS[lead.status]}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="px-6 py-4">
              <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{lead.campaignId}</dd>
                </div>
                <div>
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">
                    {lead.intakeCompletedAt ? formatDate(lead.intakeCompletedAt) : 'Not completed'}
                  </dd>
                </div>
              </dl>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="px-6 py-4">
              <dl className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <dd className="text-sm font-medium text-gray-900">{lead._count.notes}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Documents</dt>
                  <dd className="text-sm font-medium text-gray-900">{lead._count.documents}</dd>
                </div>
                <div className="flex justify-between">
                  <dt className="text-sm text-gray-500">Follow-ups</dt>
                  <dd className="text-sm font-medium text-gray-900">{lead._count.followupQueue}</dd>
                </div>
              </dl>
            </div>
          </div>
          ...
          {lead.intakeToken && (
            <div className="bg-white shadow rounded-lg">
              <div className="px-6 py-4 border-b border-gray-200">
                <h2 className="text-lg font-medium text-gray-900">Intake Link</h2>
              </div>
              <div className="px-6 py-4">
                <div className="space-y-3">
                  <p className="text-sm text-gray-600">
                    Share this link with the prospect to complete their application:
                  </p>
                  <div className="flex items-center space-x-2">
                    <input
                      type="text"
                    ...
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(`${window.location.origin}/application/${lead.intakeToken}`)
