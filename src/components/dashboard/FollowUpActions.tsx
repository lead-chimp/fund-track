...
export function FollowUpActions({
  ...
  const handleRestartFollowUps = async () => {
    if (!confirm("Are you sure you want to restart the follow-up sequence? This will cancel any pending follow-ups and schedule new ones.")) {
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to restart follow-ups");
      }

      const data = await response.json();
      alert(`Follow-up sequence restarted successfully. ${data.scheduledCount} follow-ups scheduled.`);
      ...
      alert(error instanceof Error ? error.message : "Failed to restart follow-ups");
    } finally {
  ...
  const handleSendIntakeEmail = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : "Failed to send intake email";
      
      // Show more helpful error messages and set persistent error state
      if (errorMessage.includes("not properly configured")) {
        setEmailConfigError("Email service is not configured. Please contact your system administrator.");
      } else if (errorMessage.includes("Mailgun client not initialized")) {
        setEmailConfigError("Email service is not available. Please contact your system administrator.");
      } else {
  ...
  return (
    <RoleGuard allowedRoles={["ADMIN" as UserRole, "USER" as UserRole, "SYSTEM_ADMIN" as UserRole]} fallback={<></>}>
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          <h2 className="text-lg font-medium text-gray-900">Follow-Up Actions</h2>
        </div>
        <div className="px-6 py-4 space-y-4">
          {/* Send One-Off Email */}
          <div>
            <h3 className="text-sm font-medium text-gray-900 mb-2">Send Intake Email</h3>
          ...
          <div className="pt-4 border-t border-gray-200">
            <h3 className="text-sm font-medium text-gray-900 mb-2">Restart Follow-Up Sequence</h3>
            <p className="text-sm text-gray-600 mb-3">
              Cancel any pending follow-ups and restart the automated email sequence (3h, 9h, 24h, 72h).
            </p>
            <button
            ...
            {hasFollowUps && (
              <p className="text-xs text-yellow-600 mt-1">
                ⚠️ This will cancel {hasFollowUps ? "existing" : "any"} pending follow-ups
              </p>
            )}
          </div>
          ...
          <div className="pt-4 border-t border-gray-200">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
              <div className="flex">
                ...
                <div className="ml-3">
                  <h3 className="text-sm font-medium text-blue-800">Follow-Up Information</h3>
                  <div className="mt-2 text-sm text-blue-700">
                    <ul className="list-disc list-inside space-y-1">
                      <li>Follow-ups are only sent to leads with PENDING status</li>
