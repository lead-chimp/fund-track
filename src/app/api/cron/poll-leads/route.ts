...
export async function POST(request: NextRequest) {
  try {
    ...
    if (pollingResult.newLeads > 0) {
      logger.info(`Processing notifications for ${pollingResult.newLeads} new leads`);
    ...
    logger.error('Lead polling process failed', { error: error instanceof Error ? error.message : 'Unknown error' });
