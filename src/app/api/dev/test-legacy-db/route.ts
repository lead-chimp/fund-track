...
export async function POST(request: NextRequest) {
  try {
    ...
    if (customValues) {
      if (customValues.Phone && !validatePhoneNumber(customValues.Phone)) {
        return NextResponse.json(
          {
            error:
              "Invalid Phone number format - must be 10 digits (e.g., 7488883486)",
          },
          { status: 400 }
        );
      }
      ...
      if (
        customValues.AlternatePhone &&
        !validatePhoneNumber(customValues.AlternatePhone)
      ) {
        return NextResponse.json(
          {
            error:
              "Invalid AlternatePhone number format - must be 10 digits (e.g., 7488883486)",
          },
          { status: 400 }
        );
      }
