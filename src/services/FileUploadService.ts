...
export class FileUploadService {
    ...
    validateFile(
        ...
        if (file.length > validation.maxSizeBytes) {
            throw new Error(`File size exceeds maximum allowed size of ${validation.maxSizeBytes} bytes`);
        }
     ...
     */
    private generateUniqueFileName(originalFileName: string, leadId: number): string {
        const timestamp = Date.now();
        const hash = createHash('md5').update(`${leadId}-${originalFileName}-${timestamp}`).digest('hex').substring(0, 8);
     ...
     */
    async getDownloadUrl(fileId: string, fileName: string, expirationHours: number = 24): Promise<FileDownloadResult> {
        try {
            await this.initialize();
