...
async function emergencyCleanup() {
  try {
    ...
    console.log(
      `[Cleanup] Total notifications before cleanup: ${totalBefore.toLocaleString()}`,
    );

    if (totalBefore === 0) {
      console.log("[Cleanup] No notifications to clean up.");
      return;
    }
    ...
    if (excessiveLeads.length > 0) {
      console.log("\n[Cleanup] Leads with excessive notifications (>100):");
      excessiveLeads.forEach((lead) => {
        console.log(
          `  ${lead.email}: ${Number(lead.notification_count).toLocaleString()} notifications`,
        );
      });
    }
    ...
    if (totalBefore > 1000) {
      console.log(
        `\n[Cleanup] WARNING: About to delete ${totalBefore.toLocaleString()} notification records!`,
      );
      ...
      if (!isEmergency) {
        console.log(
          "[Cleanup] Emergency cleanup not confirmed. Set EMERGENCY_CLEANUP=true to proceed.",
        );
        console.log(
          "Example: EMERGENCY_CLEANUP=true node scripts/emergency-cleanup.mjs",
        );
        return;
      }
    }
    ...
    for (const lead of leadsWithNotifications) {
      ...
      if (deleteResult.count > 0) {
        console.log(
          `  Cleaned lead ${leadId}: deleted ${deleteResult.count} notifications`,
        );
      }
    }
    ...
    console.log("\n[Cleanup] Emergency cleanup completed!");
    console.log(
      `[Cleanup] Total notifications after cleanup: ${totalAfter.toLocaleString()}`,
    );
    console.log(`[Cleanup] Total deleted: ${totalDeleted.toLocaleString()}`);
    console.log(
      `[Cleanup] Space saved: ${(((totalBefore - totalAfter) / totalBefore) * 100).toFixed(1)}%`,
    );
  } catch (error) {
