...
async function main() {
  ...
  if (isProduction && !forceSeeding) {
    ...
    console.error(
      "   If you really need to seed in production, use: FORCE_SEED=true npm run db:seed"
    );
  ...
  if ((existingUsers > 0 || existingLeads > 0) && !forceSeeding) {
    ...
    console.error(
      `   Found ${existingUsers} users and ${existingLeads} leads.`
    );
    console.error(
      "   To force seeding (THIS WILL DELETE ALL DATA), use: FORCE_SEED=true npm run db:seed"
    );
    process.exit(1);
  }
  ...
  if (forceSeeding) {
    console.log(
      "⚠️  FORCE_SEED enabled - proceeding with data deletion and seeding"
    );
    if (isProduction) {
      console.log(
        "⚠️  PRODUCTION ENVIRONMENT - This will delete all production data!"
      );
  ...
  const regularUser = await prisma.user.create({
    data: {
      email: "user-001@merchantfunding.com",
  ...
  const salesUser = await prisma.user.create({
    data: {
      email: "user-002@merchantfunding.com",
      passwordHash: userPassword,
      role: UserRole.USER,
    },
  });
  ...
  await Promise.all([
    prisma.leadNote.create({
      data: {
        ...
        userId: adminUser.id,
        content:
          "Initial review completed. Documents look good, proceeding with verification.",
      },
    }),
    prisma.leadNote.create({
      data: {
        ...
        content:
          "Called customer to verify business information. All details confirmed.",
      },
    }),
    prisma.leadNote.create({
      data: {
        ...
        userId: adminUser.id,
        content:
          "Application approved. Funding amount: $50,000. Terms: 12 months.",
      },
    }),
    prisma.leadNote.create({
      data: {
        ...
        userId: salesUser.id,
        content:
          "Application rejected due to insufficient credit score. Advised to reapply in 6 months.",
      },
    }),
  ]);
  ...
  await Promise.all([
    ...
    prisma.notificationLog.create({
      data: {
        ...
        content:
          "Thank you for your interest in merchant funding. Please complete your application at: http://localhost:3000/application/token_new_lead_001",
    ...
    prisma.notificationLog.create({
      data: {
        ...
        subject: null,
        content:
          "Complete your merchant funding application: http://localhost:3000/application/token_new_lead_001",
        status: NotificationStatus.SENT,
    ...
    prisma.notificationLog.create({
      data: {
        ...
        content:
          "This is a friendly reminder to complete your merchant funding application.",
        status: NotificationStatus.SENT,
  ...
  console.log("   User: user-001@merchantfunding.com / user123");
